...
export class Database {
  ...
  ensureMigrated() {
    ...
    if (this.storedSchemaVersion == latestSchemaVersion) {
      console.log("already migrated up");
      return;
    }
      ...
      for (const focus of ["cnt", "global"]) {
        ...
        for (const [studyAreaName, projects] of schemaV0_listProjects(
          ...
          for (const { projectKey, projectName } of projects) {
            ...
            let id = projectStorage.createNewProject(projectSummary);
            console.log(
...
export class ProjectStorage {
  ...
  private get collectionKey() {
    return `ltn/${this.appFocus}/${this.collectionName}`;
  }
   ...
   */
  createNewProject(projectSummary: ProjectSummary): ProjectID {
    if (this.projectNameAlreadyExists(projectSummary.projectName)) {
      throw new Error(
        `Project name already taken: ${projectSummary.projectName}`,
      );
    }
